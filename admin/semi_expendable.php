    ...
    if (empty($errors)) {
        ...
        try {
            if ($generate_multiple && $quantity > 1 && floor($quantity) == $quantity) {
                ...
                // First, get the codes for building the property number
                $type_query = $conn->prepare("SELECT code FROM type_of_equipment WHERE id = ?");
                $type_query->bind_param("i", $type_equipment_id);
                $type_query->execute();
                $type_result = $type_query->get_result();
                ...
                for ($i = 1; $i <= $quantity; $i++) {
                    ...
                    $stmt = $conn->prepare("
                        INSERT INTO inventory (
                            ...
                            equipment_id, section_id, category, type_equipment_id, equipment_sub_type_id, condition_text,
                            fund_cluster, certified_correct, approved_by, verified_by,
                            allocate_to, remarks, barcode_data, created_by, year_acquired, date_added, date_updated
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                    ");
                    ...
                    $stmt->bind_param(
                        "ssssddiiiiissiiiiissii",
                        $article_name, $description, $property_no, $uom,
                        $qty_property_card, $qty_physical_count, $unit_value,
                        $equipment_id, $section_id, $category, $type_equipment_id, $equipment_sub_type_id, $condition_text,
                        $fund_cluster, $certified_correct, $approved_by, $verified_by,
                        $allocate_to, $remarks, $sequential_barcode, $created_by, $year_acquired
                    );
                ...
                $stmt->bind_param(
                    ...
                    $equipment_id, $section_id, $category, $type_equipment_id, $equipment_sub_type_id, $condition_text,
                    $fund_cluster, $certified_correct, $approved_by, $verified_by,
                    $allocate_to, $remarks, $barcode_data, $created_by, $year_acquired
                );
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'edit' && isset($_POST['id'])) {
    ...
    $stmt = $conn->prepare("
        UPDATE inventory SET 
            ...
            equipment_id = ?, section_id = ?, type_equipment_id = ?, equipment_sub_type_id = ?,
    ...
    $stmt->bind_param(
        ...
        $equipment_id, $section_id, $type_equipment_id, $equipment_sub_type_id, $condition_text,
        $fund_cluster, $certified_correct, $approved_by, $verified_by,
...
$query = "
    ...
    LEFT JOIN users al ON i.allocate_to = al.id
    LEFT JOIN users cr ON i.created_by = cr.id
    WHERE i.category = 'Semi-Expendable'
";

$count_query = "SELECT COUNT(*) as total FROM inventory WHERE category = 'Semi-Expendable'";
...
<div class="dashboard-cards">
    ...
    <div class="card">
        ...
        $stmt = $conn->prepare("
            ...
            LEFT JOIN type_of_equipment toe ON i.type_equipment_id = toe.id
            WHERE i.category = 'Semi-Expendable' AND ei.status = 'issued'
        ");
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    <div class="card">
        ...
        $stmt = $conn->prepare("
            ...
            FROM inventory i
            LEFT JOIN type_of_equipment toe ON i.type_equipment_id = toe.id
            WHERE i.category = 'Semi-Expendable' AND i.qty_physical_count <= 5
        ");
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        <tbody>
            <?php if (count($semi_items) > 0): ?>
                ...
                <tr class="<?php echo $item['qty_physical_count'] <= 5 ? 'stock-alert-row' : ''; ?>">
                    ...
                    <td><?php echo htmlspecialchars($item['section_name'] ?? 'N/A'); ?></div>
...
function loadEquipmentSubTypes() {
    ...
    // Make AJAX request to get sub types
...
function previewMultipleBarcodes() {
    ...
    // Get the type code and sub type code for better barcode
    ...
    fetch('?ajax=get_item&id=' + id)
        ...
        .then(data => {
            if (data.success) {
                ...
                let typeDisplay = '';
                if (item.type_equipment_name) {
                    typeDisplay = item.type_equipment_name;
                    if (item.sub_type_name) {
                        typeDisplay += ' - ' + item.sub_type_name;
                    }
                } else {
                    typeDisplay = item.category || 'Semi-Expendable';
                }
                        ...
                        <div class="detail-content">
                            ...
                            <div class="barcode-detail-item">
                                ...
                                <div class="barcode-image">
                                    <img src="generate_barcode.php?code=${encodeURIComponent(item.barcode_data)}&format=png&width=300&height=60" 
                                         alt="Barcode" 
    ...
    printWindow.document.write(`
        ...
        <head>
            ...
            <style>
                ...
                .semi-label { background: #6B8CFF; color: white; padding: 5px 15px; border-radius: 20px; display: inline-block; font-size: 14px; margin-bottom: 15px; }
        ...
        <body>
            <div class="barcode-container">
                ...
                <img src="generate_barcode.php?code=${encodeURIComponent(barcodeData)}&format=png&width=400&height=100&t=${timestamp}" 
    ...
    fetch('?ajax=get_edit_item&id=' + editId)
        ...
        .then(data => {
            if (data.success) {
                ...
                // Set type of equipment dropdown
                if (item.type_equipment_id) {
                    let typeSelect = document.getElementById('type_equipment_id');
                    typeSelect.value = item.type_equipment_id;
                    // Load sub types and then set the sub type
