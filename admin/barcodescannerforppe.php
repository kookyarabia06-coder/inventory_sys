            ...
            <div class="scanner-card">
                    ...
                    <div id="liveSearchSection" class="search-input-section">
                        ...
                        <div class="input-group">
                            ...
                            <button class="btn btn-primary" onclick="searchLiveBarcode()">
                                <i class="fas fa-search"></i> Find PPE
                            </button>
                        </div>
                    </div>
                ...
                <div id="liveScannerSection" class="camera-section">
                    ...
                    <div class="scanner-instructions">
                        <p><i class="fas fa-info-circle"></i> Position the barcode in the center of the red line. The scanner will automatically detect PPE barcodes.</p>
                    </div>
                </div>
                ...
                <div class="recent-scans">
                    <h4><i class="fas fa-history"></i> Recent PPE Scans</h4>
...
function performBarcodeSearch(barcode) {
    ...
    document.getElementById('scanResult').innerHTML = '<div class="loading"><i class="fas fa-spinner fa-spin"></i><br>Searching for PPE item...</div>';
    ...
    Quagga.init({
        ...
        Quagga.onDetected(function(result) {
            ...
            if (code) {
                ...
                document.getElementById('scanner-status').textContent = 'PPE Barcode detected: ' + code;
    ...
    let html = `
        <div class="found-item">
            <div class="item-detail-card">
                <h3>${escapeHtml(item.article_name)} <span class="ppe-badge">PPE</span></h3>
...
function displayNotFound(barcode) {
    let html = `
        <div class="not-found">
            <i class="fas fa-shield-alt"></i>
            <h3>PPE Item Not Found</h3>
            <p>No PPE item found with barcode: <strong>${escapeHtml(barcode)}</strong></p>
            <div style="display: flex; gap: 10px; justify-content: center;">
                <button class="btn btn-primary" onclick="openAddPPEWithBarcode('${escapeHtml(barcode)}')">
                    <i class="fas fa-plus"></i> Add New PPE
                </button>
...
function addToRecentScans(item) {
    ...
    try {
        localStorage.setItem('ppeRecentScans', JSON.stringify(recentScans));
    } catch (e) {
...
function displayRecentScans() {
    ...
    if (!recentScans || recentScans.length === 0) {
        html = '<div class="text-muted" style="padding: 10px; text-align: center;">No recent PPE scans</div>';
    } else {
