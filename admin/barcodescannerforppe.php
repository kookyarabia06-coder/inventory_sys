 ...
 * Scan barcodes to quickly find PPE items
 */
...
if (!isset($conn) || !$conn) {
    error_log("Database connection failed in barcodescannerforppe.php");
...
$page_title = 'PPE Barcode Scanner';
$page_description = 'Scan barcodes to quickly find PPE items';
    ...
    try {
        ...
        $issued_query = "
            SELECT 
                ...
                i.big_quantity,
                i.small_unit,
                i.pieces_per_big_unit,
                i.unit_value,
                i.fund_cluster_id,
                i.barcode_data,
                i.type_equipment_id,
                i.equipment_sub_type_id,
            ...
            WHERE ei.issuance_barcode = ? AND i.category = 'PPE'
            LIMIT 1
        ";
        ...
        // If not found in issued items, search in inventory table
        if (!$row) {
            $inventory_query = "
                SELECT 
                    i.id,
                    ...
                    NULL as status
                FROM inventory i
                WHERE (i.barcode_data = ? OR i.property_no = ?) AND i.category = 'PPE'
                LIMIT 1
            ";
        ...
        if ($row) {
            ...
            $fund_cluster_name = 'N/A';
            if (!empty($row['fund_cluster_id'])) {
                $fc_stmt = $conn->prepare("SELECT name FROM fund_cluster WHERE id = ?");
                $fc_stmt->bind_param("i", $row['fund_cluster_id']);
                $fc_stmt->execute();
                $fc_result = $fc_stmt->get_result();
                if ($fc_row = $fc_result->fetch_assoc()) {
                    $fund_cluster_name = $fc_row['name'];
                }
                $fc_stmt->close();
            }
            
            // Get type equipment name
            $type_equipment_name = 'PPE';
            if (!empty($row['type_equipment_id'])) {
                $te_stmt = $conn->prepare("SELECT name FROM type_of_equipment WHERE id = ?");
                $te_stmt->bind_param("i", $row['type_equipment_id']);
                $te_stmt->execute();
                $te_result = $te_stmt->get_result();
                if ($te_row = $te_result->fetch_assoc()) {
                    $type_equipment_name = $te_row['name'];
                }
                $te_stmt->close();
            }
            ...
            echo json_encode([
                ...
                'item' => [
                    ...
                    'big_unit' => htmlspecialchars($row['big_unit'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                    'small_unit' => htmlspecialchars($row['small_unit'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                    'quantity' => $quantity,
                    'unit_value' => (float)($row['unit_value'] ?? 0),
                    'condition_text' => htmlspecialchars($row['condition_text'] ?? 'Good', ENT_QUOTES, 'UTF-8'),
                    'fund_cluster' => htmlspecialchars($fund_cluster_name, ENT_QUOTES, 'UTF-8'),
                    'type_equipment' => htmlspecialchars($type_equipment_name, ENT_QUOTES, 'UTF-8'),
                    'is_issued' => $is_issued,
            ...
            echo json_encode([
                'success' => true,
                'found' => false,
                'message' => 'No PPE item found with this barcode',
                'barcode' => htmlspecialchars($barcode, ENT_QUOTES, 'UTF-8')
            ]);
        }
        
    } catch (Exception $e) {
        error_log("PPE Barcode scanner error: " . $e->getMessage());
...
<!-- Scanner Page - Same as Semi-Expendable -->
<style>
...
<div class="scanner-container">
    <div class="scanner-header">
        <h2><i class="fas fa-box-open"></i> PPE Barcode Scanner</h2>
        <p>Scan barcodes to quickly find PPE items</p>
    </div>
    ...
    <div class="scanner-main">
        ...
        <div class="scanner-left">
            ...
            <div class="info-text" style="margin-top: 20px; padding: 15px; background: #F0F0F0; border-radius: 8px;">
                ...
                <small>• Original item barcode (from inventory table)</small><br>
        ...
        <div class="scanner-right">
            <div id="scanResult" class="scan-result">
                <div class="scan-placeholder"><i class="fas fa-box-open"></i><p>Scan a PPE barcode to see item details</p></div>
            </div>
...
function displayItem(item) {
            ...
            <div class="issued-info">
                ...
                <div class="item-detail-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <span class="detail-value highlight">${escapeHtml(item.property_no)}</span>
                    </div>
                </div>
            </div>
        `;
    ...
    let html = `<div class="found-item">
        <div class="item-detail-card">
            <h3>${escapeHtml(item.article_name)} <span style="background:#F8B0C0;padding:3px 10px;border-radius:15px;font-size:12px;">PPE</span></h3>
            ${issuedInfo}
            <div class="item-detail-grid">
                ...
                <div class="detail-item"><span class="detail-label">Equipment Type</span><span class="detail-value">${escapeHtml(item.type_equipment)}</span></div>
                <div class="detail-item"><span class="detail-label">Big Unit</span><span class="detail-value">${escapeHtml(item.big_unit)}</span></div>
                <div class="detail-item"><span class="detail-label">Small Unit</span><span class="detail-value">${escapeHtml(item.small_unit)}</span></div>
                <div class="detail-item"><span class="detail-label">Total Quantity</span><span class="detail-value"><strong>${item.quantity}</strong></span></div>
                ...
                <div class="detail-item"><span class="detail-label">Total Value</span><span class="detail-value">₱${(item.unit_value * item.quantity).toFixed(2)}</span></div>
            ...
            <div class="barcode-preview">
                <img src="?generate_barcode=1&barcode_value=${encodeURIComponent(item.barcode_data)}" alt="Barcode" onerror="this.style.display='none'">
...
function displayNotFound(barcode) {
    document.getElementById('scanResult').innerHTML = `<div class="not-found"><i class="fas fa-box-open"></i><h3>Item Not Found</h3><p>No PPE item found with barcode: <strong>${escapeHtml(barcode)}</strong></p><button class="btn-secondary" onclick="resetScanner()">Try Again</button></div>`;
...
function resetScanner() {
    document.getElementById('scanResult').innerHTML = '<div class="scan-placeholder"><i class="fas fa-box-open"></i><p>Scan a PPE barcode to see item details</p></div>';
    document.getElementById('barcode_input').value = '';
    document.getElementById('barcode_input').focus();
}
    ...
    if (currentScannedItem) {
        ...
        printWindow.document.write(`
            ...
            <style>
                ...
                .ppe-label{background:#F8B0C0;padding:5px 15px;border-radius:20px;display:inline-block;margin-bottom:15px}
            </style>
            </head>
            <body>
                <div class="barcode-container">
                    <div class="ppe-label">PPE Equipment</div>
                    <img src="?generate_barcode=1&barcode_value=${encodeURIComponent(currentScannedItem.barcode_data)}" alt="Barcode">
                    <div style="margin-top:15px;font-weight:bold">${escapeHtml(currentScannedItem.article_name)}</div>
                    <div style="font-family:monospace;margin-top:10px">${escapeHtml(currentScannedItem.barcode_data)}</div>
