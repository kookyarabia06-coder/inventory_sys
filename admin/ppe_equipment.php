...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] == 'add') {
        ...
        if (empty($errors)) {
            ...
            // article_name, description, property_no, uom (4)
            // qty_property_card, qty_physical_count, unit_value (3 more = 7)
            // equipment_id, section_id, category, type_equipment, condition_text (5 more = 12)
            // fund_cluster, certified_correct, approved_by, verified_by (4 more = 16)
            // allocate_to, remarks, created_by (3 more = 19)
            // Total: 19 parameters
            
            $stmt->bind_param(
                "sssdddiiisssssiiiisi",  // 19 characters: 11 s, 3 d, 5 i
                $article_name,           // s
                $description,             // s
                $property_no,              // s
                $uom,                      // s
    ...
    elseif ($_POST['action'] == 'edit' && isset($_POST['id'])) {
        ...
        $stmt = $conn->prepare("
            UPDATE inventory SET 
                ...
                remarks = ?, date_updated = NOW()
            WHERE id = ?
        ");
        
        // Count parameters: 16 SET + 1 WHERE = 17
        $stmt->bind_param(
            "sssddiissssiiiisi",  // 17 characters: 9 s, 2 d, 6 i
            $article_name,           // s
            $description,             // s
            $uom,                      // s
            $quantity,                 // d
